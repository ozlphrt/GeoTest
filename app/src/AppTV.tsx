...
function AppTV() {
  ...
  // TV Input Handling
  useEffect(() => {
    // Auto-focus logic for MCQs
    if (currentQuestion && currentQuestion.type !== 'map_tap') {
      setTimeout(() => {
        const firstOption = document.querySelector('.options button') as HTMLElement
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
        ...
        if (isButtonFocused || (!isMapTap && !isButtonFocused)) {
          const active = document.activeElement as HTMLElement

          // RESCUE FOCUS: If focus is lost (e.g. on body), any arrow key grabs it.
          if (!isButtonFocused) {
            const firstOption = document.querySelector('.options button') as HTMLElement
            if (firstOption) {
              firstOption.focus()
            ...
            // If no options, try powerups
            const firstPowerup = document.querySelector('.powerup-btn:not(:disabled)') as HTMLElement
            if (firstPowerup) {
              firstPowerup.focus()
              return
            }
          ...
          // Identify zones
          const isOption = active.classList.contains('option')

          if (isOption) {
            // GRID NAVIGATION FOR OPTIONS
            const options = Array.from(document.querySelectorAll('.options button')) as HTMLElement[]
            const idx = options.indexOf(active)
            const numCols = 2 // Fixed 2-column grid from CSS
            let nextIdx = idx

            if (e.key === 'ArrowUp') {
              if (idx - numCols >= 0) nextIdx = idx - numCols
              else {
                // Escape Grid Upwords -> Powerups
                const powerups = Array.from(document.querySelectorAll('.powerup-btn:not(:disabled)')) as HTMLElement[]
                if (powerups.length) {
                  powerups[0].focus()
                  return
                }
              ...
              if (idx + numCols < options.length) nextIdx = idx + numCols
            } else if (e.key === 'ArrowLeft') {
              if (idx % numCols !== 0) nextIdx = idx - 1
            } else if (e.key === 'ArrowRight') {
              if ((idx + 1) % numCols !== 0 && idx + 1 < options.length) nextIdx = idx + 1
            }

            if (nextIdx !== idx) options[nextIdx].focus()

          } else {
            // NAVIGATION FOR OTHER ELEMENTS (Linear fallback or specific)
            // Just collect everything else and do smart-ish linear
            const others = Array.from(document.querySelectorAll('.utility-controls button, .powerup-btn:not(:disabled)')) as HTMLElement[]
            // Sort usually by DOM order? Settings -> Powerups
            // Append options[0] as the "Down" target for the last item

            const currentIdx = others.indexOf(active)

            if (currentIdx !== -1) {
              if (e.key === 'ArrowDown') {
                if (currentIdx + 1 < others.length) others[currentIdx + 1].focus()
                else {
                  // Down from last "Other" -> First Option
                  const firstOption = document.querySelector('.options button') as HTMLElement
                  if (firstOption) firstOption.focus()
                }
              } else if (e.key === 'ArrowUp') {
                if (currentIdx - 1 >= 0) others[currentIdx - 1].focus()
              } else if (e.key === 'ArrowRight') {
                // Allow Right to behave like Down for linear lists (powerups are horizontal though)
                if (currentIdx + 1 < others.length) others[currentIdx + 1].focus()
              } else if (e.key === 'ArrowLeft') {
                if (currentIdx - 1 >= 0) others[currentIdx - 1].focus()
              }
          ...
          // MAP MODE: Go country by country (Spatial Snap)
          // We use the featureIndex (loaded GeoJSON) to find the nearest country centroid in the direction
          const currentCenter = map.getCenter()
          let bestCandidate: { center: [number, number], dist: number } | null = null

          // Define target angle ranges (in degrees, 0 is East/Right, 90 is North/Up) - Math.atan2 returns radians
          // ArrowRight: 0 (+/- 45)
          // ArrowUp: 90 (+/- 45)
          // ArrowLeft: 180 (+/- 45)
          // ArrowDown: -90 (+/- 45)
          ...
          switch (e.key) {
            ...
            case 'ArrowUp': targetAngle = Math.PI / 2; break; // +90 deg
            case 'ArrowLeft': targetAngle = Math.PI; break;   // 180 deg
            case 'ArrowDown': targetAngle = -Math.PI / 2; break; // -90 deg
          }
          ...
          for (const record of featureIndex.values()) {
            ...
            // Simple Euclidean dist/angle in equirectangular (good enough for nav)
            const dx = cX - currentCenter.lng
            const dy = cY - currentCenter.lat

            // Skip if too close (current country)
            ...
            // Angle diff logic (handle wrap around PI)
            let angleDiff = angle - targetAngle
            while (angleDiff <= -Math.PI) angleDiff += 2 * Math.PI
            while (angleDiff > Math.PI) angleDiff -= 2 * Math.PI

            // Cone of 60 degrees (PIOver3) either side
            if (Math.abs(angleDiff) < Math.PI / 3) {
              // Verify it's a valid candidate
              // Prefer closer ones
              if (!bestCandidate || distSq < bestCandidate.dist) {
                bestCandidate = { center: [cX, cY], dist: distSq }
              }
            }
          }
          ...
          if (bestCandidate) {
            map.flyTo({
              ...
              essential: true,
              duration: 400, // Quick snap
              padding: { top: 50, bottom: 50, left: 50, right: 50 } // Keep away from edges
            })
          } else {
            // Fallback: minimal pan if no country found (e.g. edge of world)
            const panSpeed = 300
            let pdx = 0, pdy = 0
            if (e.key === 'ArrowUp') pdy = -panSpeed
            else if (e.key === 'ArrowDown') pdy = panSpeed
            else if (e.key === 'ArrowLeft') pdx = -panSpeed
            else if (e.key === 'ArrowRight') pdx = panSpeed
            map.panBy([pdx, pdy], { duration: 300 })
          }
        }
      }

      // Select / Enter Key
      if (e.key === 'Enter' || e.key === 'Select') { // Fire TV often uses 'Select' or 'Enter'
        if (isButtonFocused) {
          // Let default click happen (React onClick) provided browser handles it. 
          // Some browsers don't fire click on Enter for buttons, so we force it.
          (document.activeElement as HTMLButtonElement).click()
          return
        }

        // Trigger Map Tap at Center (Only if in map_tap mode)
        if (isMapTap) {
          const center = map.getCenter()
          const point = map.project(center)

          // Synthetic Event for handleMapTap
          const syntheticEvent = {
            lngLat: center,
            point: point,
            originalEvent: e as unknown as MouseEvent // Cast to satisfy type
          } as maplibregl.MapMouseEvent

          map.fire('click', syntheticEvent)
        }
      }
    }
  ...
  }, [currentQuestion, featureIndex]) // Re-bind when question changes to ensure state is fresh
  ...
  return (
    <div className={`app ${mapShake ? 'animate-map-shake' : ''}`}>
      ...
      <div className="map-reticle">
        <div className="map-reticle-circle" />
      </div>
            ...
            <div className="stat-group">
              <div className="circular-container">
                <svg className="circular-progress">
                  <circle className="circular-bg" cx="40" cy="40" r="36" />
                  <circle
                    className={`circular-value ${isLevelUp ? 'animate-pulse' : ''} `}
                    cx="40" cy="40" r="36"
                    strokeDasharray="226.2"
                    strokeDashoffset={226.2 * (1 - correctInLevel / 5)}
                  />
                </svg>
